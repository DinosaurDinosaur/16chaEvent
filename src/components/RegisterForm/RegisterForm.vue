    ...
    >
      ...
      <div class="group">
        <input
          ...
          "
          :class="{ active: serialNumbers[idx]['error'] }"
        />
      </div>
      <div
        class="status"
        :class="{
          active: serialNumbers[idx]['error'] || serialNumbers[idx]['correct'],
        }"
      >
        <i
          class="far fa-times"
        ...
        ></i>
        <i
          class="fa fa-check correct"
          :class="{ active: serialNumbers[idx]['correct'] }"
        ></i>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      serialNumbers: [
        { value: "", error: false, correct: false },
        { value: "", error: false, correct: false },
        { value: "", error: false, correct: false },
        { value: "", error: false, correct: false },
        { value: "", error: false, correct: false },
  ...
  methods: {
    ...
    async submit() {
      ...
      if (!this.agree) return this.showCheckPopup();
    ...
    showCheckPopup() {
      this.$store.dispatch("showModal", "register/check");
    },
    ...
    showValidateFailIcon(checkList) {
      checkList.forEach((itm, idx) => {
        // if (itm !== 2) return;
        if (itm === 1) {
          this.serialNumbers[idx].correct = true;
        }
        if (itm === 2) {
          this.serialNumbers[idx].error = true;
        }
        if (itm === 0) {
          this.serialNumbers[idx].correct = false;
          this.serialNumbers[idx].error = false;
        }
      });
    },
